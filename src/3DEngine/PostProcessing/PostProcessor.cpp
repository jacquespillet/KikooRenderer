...
void PostProcessor::Run(Framebuffer* framebufferIn, Framebuffer* framebufferOut) {
    ...
    for (int i = 0; i < processes.size(); i++) {
        if(i==0)    processes[i]->Run(framebufferIn, internalFBO1);
        else {
    ...
    if(processes.size() % 2 == 0) framebufferOut = internalFBO1;
    else framebufferOut = internalFBO0;
